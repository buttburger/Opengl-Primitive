...
char Load3DS(obj_type_ptr p_object, char *p_filename)
{
	...
	{
        //MessageBox(NULL,"3DS file not found","Stuff", MB_OK|MB_ICONERROR);
		return (0);
	}
	while(ftell (l_file) < filelength (fileno (l_file))) //Loop to scan the whole file 
	{
		...
		switch (l_chunk_id)
        {
			...
			case 0x4110: 
				...
				{
					//MessageBox(NULL,"Number of vertices too high", "Stuff", MB_OK|MB_ICONERROR);
					return (0);
				}
                for(i=0; i<l_qty; i++)
                {
			...
			case 0x4120:
				...
				{
			        //MessageBox(NULL, "Number of polygons too high","Stuff", MB_OK|MB_ICONERROR);
					return(0);
				}				
                for (i=0; i<l_qty; i++)
                {
